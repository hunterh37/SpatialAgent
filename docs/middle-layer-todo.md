...
## Found by running the whole thing (phase 2.1)

A local model, `agentd`, and the visionOS app in the simulator, all at once. Every item
here was invisible to the mocked tests on either side:

- **`deviceStates` from visionOS is empty and was believed.** The app has no HomeKit, so it
  sent `devices: []`, which wiped the server-owned home out of the prompt and left the
  model with nothing to act on. A server-owned list is now authoritative.
- **The client had no way to learn a server-owned device's name.** Added `homeDevices`
  (server -> client), which `RemoteHomeProvider.ingest` was already written to receive. The
  confirmation ornament can name the lock it is asking about.
- **Nested `ObservableObject`s do not propagate.** `ConfirmationGate` published into nobody:
  the view observed `AgentSession`, so an unsafe call sat invisible until it timed out.
  `AgentSession.pendingConfirmations` mirrors the gate.
- **The client's confirmation window and the server's wait were both 30s**, so a human's tap
  could land on a call the server had already abandoned. `AGENTD_CONFIRM_TIMEOUT` is 120s.
- **Movement inferred from prose is unreliable.** `walk_to` and `look_at` are tools now, so
  the model states the intent rather than narrating it and hoping a regex matches.
- **Tool schemas were rejected outright by Ollama** (`required` inside a property), so no
  tool had ever been called by a real model.
- **Sampling.** 0.2 by default; at higher temperatures a 3B model narrates instead of acting.

Coverage added with them: `packages/Tests/LiveIntegrationTests` (the real Swift stack over a
real socket, `make live`), `apps/SpatialAgent/UITests` (the app driven through its UI in the
visionOS simulator, `make live-app`), and a cross-language conformance corpus generated by
`agentd.conformance` that both languages decode and assert on.
